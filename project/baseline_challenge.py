...
labeling_function = lambda x: 1 if x > 4 else 0
...
class BaselineChallenge(FlowSpec):
    ...
    def baseline(self):
        ...
        acc = model.eval_acc(X = self.valdf['review'], labels = self.valdf['label']) # TODO: return the accuracy_score of these predictions
         
        rocauc = model.eval_rocauc(X = self.valdf['review'], labels = self.valdf['label'])  # TODO: return the roc_auc_score of these predictions
